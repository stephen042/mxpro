        ...
        <table class="w-full text-sm text-left text-gray-900 dark:text-gray-300">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                    ...
                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-gray-100">${{ number_format($user->balance, 2) }}</td>
                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-gray-100">${{ number_format($user->sub_balance, 2) }}</td>
                    <!-- Action buttons -->
                    <td class="px-6 py-4">
                        <div class="flex space-x-2">
                            ...
                            <!-- Delete Button -->
                            <button type="button" wire:click="deleteUser({{ $user->id }})" wire:confirm="Are you sure you want to delete this user?" class="flex items-center space-x-1 px-3 py-1 bg-red-600 text-white text-xs font-semibold rounded hover:bg-red-700 transition">
                                <!-- Trash Icon -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <button type="button" wire:click="deactivateUser({{ $user->id }})" wire:confirm="Are you sure you want to Deactivate this user?" class="flex items-center space-x-1 px-3 py-1 bg-yellow-600 text-white text-xs font-semibold rounded hover:bg-yellow-700 transition">
                                <!-- X Circle Icon -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @else
                            <button type="button" wire:click="activateUser({{ $user->id }})" wire:confirm="Are you sure you want to Activate this user?" class="flex items-center space-x-1 px-3 py-1 bg-green-600 text-white text-xs font-semibold rounded hover:bg-green-700 transition">
                                <!-- Check Circle Icon -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
